...
public class Tarea extends Application implements Runnable {
    ...
    private final Image score = new Image("/assets/blue1.png");
    ...
    private void initComponents(Stage primaryStage) {
        try {
            this.pane = new Pane();
            this.scene = new Scene(this.pane, 800, 650);
            this.canvas = new Canvas(800, 650);
            ...
            try {
                this.producer = new Producer(0, 435, buffer);
                this.consumer = new Consumer(730, 435, buffer);
    ...
    private void draw(GraphicsContext gc) {
        ...
        gc.drawImage(background, 0, 0, 800, 650);
        for (int i = 0; i < buffer.scoreCont(); i++) {
            gc.drawImage(score, i * 100, 0, 60, 70);
        }//pinta en la parte superior los barriles que se han entregado
        if (buffer.score()) {
            gc.drawImage(score, 370, 500, 60, 70);
        }//pinta el barril en el centro si es que se puso uno ahí y no lo han recogido
        gc.drawImage(this.consumer.getImage(), this.consumer.getX(), this.consumer.getY(), 80, 100);
        gc.drawImage(this.producer.getImage(), this.producer.getX(), this.producer.getY(), 80, 100);
    }
